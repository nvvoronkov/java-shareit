...
public class BookingController {
    ...
    @GetMapping
    public ResponseEntity<Object> getBookingInfoList(@RequestHeader("X-Sharer-User-Id") long userId,
    ...
    @GetMapping("/owner")
    public ResponseEntity<Object> getBookingOwnerInfoList(@RequestHeader("X-Sharer-User-Id") long userId,
                                                          @RequestParam(name = "state", defaultValue = "all") String stateParam,
                                                          @PositiveOrZero @RequestParam(name = "from", defaultValue = "0") Integer from,
                                                          @Positive @RequestParam(name = "size", defaultValue = "10") Integer size) {
        BookingState state = BookingState.from(stateParam)
                .orElseThrow(() -> new IllegalArgumentException("Unknown state: " + stateParam));
        log.info("Get booking with state {}, userId={}, from={}, size={}", stateParam, userId, from, size);
        return bookingClient.getBookingOwnerInfoList(userId, state, from, size);
    }
}
